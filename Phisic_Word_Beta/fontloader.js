...
function pathCommandToString(cmd) {
    var str =
        ((cmd.x !== undefined) ? cmd.x * wordScale + ' ' + (-cmd.y * wordScale) + ' ' : '') +
        ((cmd.x1 !== undefined) ? cmd.x1 * wordScale + ' ' + (-cmd.y1 * wordScale) + ' ' : '') +
        ((cmd.x2 !== undefined) ? cmd.x2 * wordScale + ' ' + (-cmd.y2 * wordScale) + ' ' : '');
    return str;
}
    ...
    if (glyph.numberOfContours > 0) {
        ...
        for (i in glyph.path.commands) {
            if (glyph.path.commands[i].type == "M") {
                ...
                isM = false;
            if (isM)
                WordPath += glyph.path.commands[i].x * wordScale + " " + -glyph.path.commands[i].y * wordScale + " ";
